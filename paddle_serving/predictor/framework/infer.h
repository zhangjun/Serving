...
#else
class FluidInferEngine
    : public CloneDBReloadableInferEngine<PaddleInferenceCore> {
#endif
 public:  // NOLINT
  FluidInferEngine() {}
  ~FluidInferEngine() {}
  ...
  int infer_impl(const void* in, void* out, uint32_t batch_size = -1) {
    // First of all, get the real core acording to the template parameter
    // 'PaddleInferenceCore'.
    ...
    // Set the lod and shape information of InputData first. then copy data from
    // cpu to the core.
    ...
    for (int i = 0; i < tensorVector_in_pointer->size(); ++i) {
      ...
      // Because the core needs to determine the size of memory space according
      // to the data type passed in.
      // The pointer type of data must be one of float *,int64_t*,int32_t*
      // instead void*.
    ...
    // After the input data is passed in, call 'core->Run()' perform the
    // prediction process.
    if (!core->Run()) {
    ...
    // In order to get the results, first, call the 'core->GetOutputNames()' to
    // get the name of output(which is a dict like {OutputName:pointer of
    // OutputValue}).
    ...
    // Get the type and shape information of OutputData first. then copy data to
    // cpu from the core.
    // The pointer type of data_out must be one of float *,int64_t*,int32_t*
    // instead void*.
    for (int i = 0; i < outnames.size(); ++i) {
      ...
      // Because task scheduling requires OPs to use 'Channel'(which is a data
      // structure) to transfer data between OPs.
      // We need to copy the processed data to the 'Channel' for the next OP.
      // In this function, it means we should copy the 'databuf_char' to the
      // pointer 'void* out'.(which is also called ‘tensorVector_out_pointer’)
